const STORAGE_KEY = "piece-ledger-v4-state";
const LEGACY_STORAGE_KEYS = ["piece-ledger-v3-state", "piece-ledger-v2-state", "piece-ledger-v2"];
const VISIBLE_PIECE_LIMIT = 3;
const CLOUD_TABLE = "ledger_states";
const CLOUD_CONFIG = window.PIECE_LEDGER_SYNC_CONFIG || {};
...
function bindEvents() {
  ...
  els.menuToggle.addEventListener("click", () => toggleMenu(true));
  els.menuClose.addEventListener("click", () => toggleMenu(false));
  els.menuBackdrop.addEventListener("click", () => toggleMenu(false));
...
function toggleMenu(forceOpen) {
  const isOpen = typeof forceOpen === "boolean" ? forceOpen : !document.body.classList.contains("menu-open");
  document.body.classList.toggle("menu-open", isOpen);
  els.menuToggle.setAttribute("aria-expanded", String(isOpen));
  els.appMenu.setAttribute("aria-hidden", String(!isOpen));
  els.menuBackdrop.hidden = !isOpen;

  if (isOpen) {
    els.appMenu.querySelector("[data-nav-link]")?.focus();
  } else {
...
function initCloudSync() {
  syncState.configured = isCloudConfigured();

  if (!syncState.configured) {
    setSyncStatus("Cloud nicht konfiguriert", "local");
    return;
  }

  if (!window.supabase?.createClient) {
    setSyncStatus("Supabase nicht geladen", "error");
    return;
  }

  syncState.client = window.supabase.createClient(
    CLOUD_CONFIG.supabaseUrl.trim(),
    CLOUD_CONFIG.supabaseAnonKey.trim(),
    {
      auth: {
  ...
  setSyncStatus("Cloud bereit", "local");

  syncState.client.auth.getSession().then(({ data, error }) => {
    if (error) {
    ...
    if (data.session?.user) {
      connectCloudUser(data.session.user);
    }
  });
  ...
  syncState.client.auth.onAuthStateChange((event, session) => {
    if (event === "SIGNED_OUT") {
      syncState.user = null;
      updateAuthUi();
      setSyncStatus("Abgemeldet", "local");
      return;
    }

    if (session?.user && session.user.id !== syncState.user?.id) {
      connectCloudUser(session.user);
    }
  });
...
function isCloudConfigured() {
  const url = String(CLOUD_CONFIG.supabaseUrl || "").trim();
  const key = String(CLOUD_CONFIG.supabaseAnonKey || "").trim();
  return url.startsWith("https://") && key.length > 40 && !url.includes("DEINE_") && !key.includes("DEIN_");
}
...
async function syncFromCloud() {
  ...
  try {
    ...
    if (!data) {
      await saveCloudState("Lokale Daten in Cloud gesichert.");
      return;
    }

    const remoteState = normalizeStoredState(data.data || {});
    const localState = { pieces: state.pieces, orders: state.orders };
    const remoteJson = snapshotJson(remoteState);
    const localJson = snapshotJson(localState);

    if (remoteJson === localJson) {
      setSyncStatus("Synchronisiert", "synced");
      return;
    }
    ...
    if (localHasData && remoteHasData) {
      const useRemote = window.confirm(
        "Es gibt lokale Daten und Cloud-Daten. OK lädt die Cloud-Daten. Abbrechen speichert diese lokalen Daten in die Cloud.",
      );

      if (!useRemote) {
        await saveCloudState("Lokale Daten in Cloud gespeichert.");
        return;
      }
    }
    ...
    if (remoteHasData || !localHasData) {
      ...
      setSyncStatus("Cloud-Daten geladen", "synced");
      return;
    }
...
function queueCloudSave() {
  ...
  setSyncStatus("Änderung vorgemerkt", "syncing");
  syncState.saveTimer = window.setTimeout(() => {
    saveCloudState("Synchronisiert");
  }, 700);
}

async function saveCloudState(successMessage = "Synchronisiert") {
  if (!syncState.client || !syncState.user) return;

  syncState.syncing = true;
  setSyncStatus("Speichere in Cloud...", "syncing");

  try {
    const { error } = await syncState.client.from(CLOUD_TABLE).upsert(
      {
    ...
    if (error) throw error;
    setSyncStatus(successMessage, "synced");
  } catch (error) {
    setSyncStatus(error.message || "Cloud-Speichern fehlgeschlagen", "error");
  } finally {
    syncState.syncing = false;
  }
}
...
function applyRemoteState(remoteState) {
  state.pieces = remoteState.pieces;
  state.orders = remoteState.orders;
  state.piecesExpanded = false;
...
function ensureCloudReady() {
  if (!syncState.configured) {
    setSyncStatus("Bitte zuerst sync-config.js einrichten", "error");
    return false;
  }
...
function readAuthCredentials() {
  ...
  if (!email || !password) {
    setSyncStatus("E-Mail und Passwort eingeben", "error");
    return null;
  }

  if (password.length < 6) {
    setSyncStatus("Passwort braucht mindestens 6 Zeichen", "error");
    return null;
  }
...
function updateAuthUi() {
  ...
  els.authUserLabel.textContent = signedIn ? syncState.user.email : "Nicht angemeldet";
  els.authForm.hidden = signedIn;
  els.signedInActions.hidden = !signedIn;
}

function setAuthBusy(isBusy) {
  [els.authLoginBtn, els.authSignupBtn, els.authLogoutBtn, els.syncNowBtn].forEach((button) => {
    button.disabled = isBusy;
  });
}

function setSyncStatus(message, mode) {
  const readableMessage = formatSyncMessage(message);
  els.syncStatus.textContent = readableMessage;
  els.syncStatus.title = readableMessage;
  els.syncStatus.classList.remove("local", "synced", "syncing", "error");
  els.syncStatus.classList.add(mode);
}

function formatSyncMessage(message) {
  const value = String(message || "");
  if (value.includes("ledger_states") || value.includes("schema cache")) {
    return "Supabase-Tabelle fehlt: supabase-setup.sql im SQL Editor ausführen.";
  }
  return value;
}

function hasLedgerData(value) {
  return Boolean(value?.pieces?.length || value?.orders?.length);
}
